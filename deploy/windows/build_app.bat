...
rem the path to the git directory for mousetracker
set git_dir=%2

rem the version to install
set version=%3

rem the directory that will contain the python + deps + mousetracker
set target_dir=%4
...
for /f %%i in ('dir /a:d /S /B mousetracker*') do rmdir /S /Q %%i
del /Q %target_dir%\Scripts\mousetracker

rem checkout selected version of the mousetracker project
set git_exe="C:\Program Files\Git\bin\git.exe"
cd %git_dir%
%git_exe% fetch --all
%git_exe% checkout %version%

rem build and install mousetracker using the python installed in the target directory
...
del /Q %target_dir%\mousetracker-%version%-win-amd64.exe
